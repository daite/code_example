...
def main():
    ...
    if index == 0:
        if timer == 1:
            ...
            canvas.create_text(300, 300, text="Press[Space]Key", fill="lime", tag="TITLE")

        if key == "space":
            canvas.delete("TITLE")
            canvas.create_rectangle(0, 0, 600, 400, fill="blue", tag="GAME")
            canvas.create_text(300, 150, text="Game on", fill="white", font=fnt2, tag="GAME")
            canvas.create_text(300, 300, text="[E]Game end", fill="yellow", font=fnt1, tag="GAME")
            index = 1
            timer = 0
